...
function makeLive(room_id) {
  //获取媒体源
  $.getJSON('http://59.110.140.31/services/bilibili/live/liveSource.py?cid=' + room_id + '&qn=80',function(result) {
    //错误返回
    if(result.code != 0) {
...
function getLiveRoomNumer(uid) {
  var link = 'http://59.110.140.31/services/bilibili/uploader/info.py?uid=' + uid
  var id = ''
  $.ajax({
    url: link,
